...
climate:
  - platform: cn105
    ...
    supports:
      ...
      dual_setpoint: true
      # Swing mode dropdown in climate entity (OFF, VERTICAL, HORIZONTAL, BOTH)
      # Use empty list [] to disable swing_mode dropdown
      # Defaults to ["OFF", VERTICAL, HORIZONTAL, BOTH] when omitted
      swing_mode: ["OFF", VERTICAL, HORIZONTAL, BOTH]
      # Automatically create vertical_vane_select entity with all options (AUTO, ↑↑, ↑, —, ↓, ↓↓, SWING)
      # Defaults to false when omitted
      vertical_vane_select: true
      # Automatically create horizontal_vane_select entity with all options (←←, ←, |, →, →→, ←→, SWING, AIRFLOW CONTROL)
      # Defaults to false when omitted
      horizontal_vane_select: true
